...
class BaseTrainer:
    ...
    """
    def __init__(self, metric_ftns, config, train_data_loader, valid_data_loader, 
                 len_epoch):
        self.config = config
        self.logger = config.get_logger('trainer', config['trainer']['args']['verbosity'])
        model_args = config['model']['args'] if 'model' in config else \
            config['student_model']['args']
        self.deg_flag = model_args['deg_flag']
    ...
    def _load_loss(self, config):
        """
        Load loss from the configuration file
