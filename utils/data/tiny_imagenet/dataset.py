...
def download_and_unzip(URL, root_dir):
  """
  Please download the dataset from here: http://cs231n.stanford.edu/tiny-imagenet-200.zip
  """
...
class TinyImageNetDataset(Dataset):
  def __init__(self, root_dir, train = True, transform=None, download=False, 
               preload=True, load_transform=None, max_samples=None):
    ...
    if self.preload:
      ...
      for idx in range(self.samples_num):
        s = self.samples[idx]
        img = Image.open(s[0])
        # Convert to RGB from L, values doesn't change
        if img.mode == 'L':
          img = img.convert('RGB')
